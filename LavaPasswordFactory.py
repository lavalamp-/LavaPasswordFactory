    ...
    def is_valid(self, term):
        ...
        if self._max_length:
            if len(term) > self._max_length:
                return False
        if self._illegal_chars:
            if filter(lambda x: x in self._illegal_chars):
                return False
...
class LavaLogFormatter(logging.Formatter):
    ...
    CRITICAL = RED + "[ C ] %(message)s" + NONE
    ERROR = "[" + RED + " E " + NONE + "] %(message)s"
    WARNING = "[" + YELLOW + " W " + NONE + "] %(message)s"
    INFO = "[" + GREEN + " I " + NONE + "] %(message)s"
    DEBUG = "[" + BLUE + " D " + NONE + "] %(message)s"
...
class LavaPasswordFactory(object):
    ...
    contain all of the words in the input list in lowercase, uppercase, and
    title case format (ie: ['one', 'two'] becomes ['one', 'ONE', 'One',
    'two', 'TWO', 'Two']). In offline mode, the seed_list will contain ['one',
    'ONE', 'One', 'Two', 'two', 'TWO', 'oneTWO', 'twoOne', 'oneTwo', 'twoONE',
    'ONETwo', 'TWOOne', 'TwoOne', 'TwoONE', 'TWOONE', 'Onetwo', 'Twoone',
    'ONEtwo', 'TWOone', 'ONETWO', 'OneTWO', 'onetwo', 'OneTwo', 'twoone']. As
    this is the list that all other password creation processes is based off of,
    the size of this list is exponentially (if not combinatorially) related to
    the size of the output password list.
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="LavaPasswordFactory - for all your password list generation needs."
    )
    parser.add_argument(
        "--log-level",
        required=False,
...
#TODO Check to see if initial are used anywhere
#TODO Finish filling out clean_arguments function
#TODO Fill out validate_arguments function
#TODO Write unit tests
#TODO Separate out generation+cleaning from just cleaning
#TODO Yield -> clean -> straight to file for offline mode
#TODO Throw exceptions for setters for properties that shouldn't be assigned to
#TODO Implement permutations instead of the current implementation for the appending and prepending to lists in LavaPasswordFactory
